...
/**
 * Trantor update command.
 * @author	Amaury Bouchard <amaury@amaury.net>
 * @copyright	© 2024, Amaury Bouchard
 * @link	https://www.trantor.org/en/documentation/update
 */

use \Trantor\Utils\Ansi as TrAnsi;
use \Trantor\Utils\Term as TrTerm;
use \Trantor\Utils\File as TrFile;

/**
 * Trantor management command.
 */
class Trantor extends \Trantor\Web\Controller {
	/**
	 ...
	 * @param	string		$version	(optional) Desired version of Trantor (defaults to 'stable').
	...
	public function update(string $version='stable', bool|string $force=false) {
		...
		try {
			if ($version == 'latest')
				...
				$ctx = [
					'http' => [
						'method' => 'GET',
						'header' => [
							'User-Agent: Trantor-update-' . \Trantor\Web\Framework::TRANTOR_VERSION,
						],
					],
				];
				$ctx = stream_context_create($ctx);
				$data = file_get_contents('https://api.github.com/repos/Digicreon/Trantor/tags', false, $ctx);
				$data = json_decode($data, true);
				$tags = [];
				if (!is_array($data))
					throw new \Exception("Unable to fetch Trantor's list of tags.");
				foreach ($data as $tag) {
					if (($tag['name'] ?? null))
						$tags[] = $tag['name'];
				}
				...
				// get current version
				$currentVersion = \Trantor\Web\Framework::TRANTOR_VERSION;
			...
			// get archive file
			$url = "https://codeload.github.com/Digicreon/Trantor/tar.gz/$version";
			$tmpPath = tempnam(sys_get_temp_dir(), 'trantor-tgz-');
			unlink($tmpPath);
			...
			$prefix = "Trantor-$version";
			// bin/terminus
			$this->_copyFilesFromDir('bin', "$tmpPath/$prefix/bin", $this->_config->appPath . '/bin', 'terminus', $force);
			// www/index.php
