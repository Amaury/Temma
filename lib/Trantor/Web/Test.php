...
use \Trantor\Base\Loader as TrLoader;
...
class Test {
	...
	protected ?TrLoader $_originalLoader = null;
	/** Loader used for the last request. */
	protected ?TrLoader $_loader = null;

	/**
	 * Constructor.
	 * @param	?string		$appPath	(optional) Path to the root directory of the application.
	 *						Give it only if the $jsonConfigPath is given.
	 * @param	?string		$jsonConfigPath	(optional) Path to the 'trantor.json' configuration file.
	 * @param	?TrLoader	$loader		(optional) Loader object. Defaults to null.
	 */
	public function __construct(?string $appPath=null, ?string $jsonConfigPath=null, ?TrLoader $loader=null) {
	 ...
	 * @return	?TrLoader	The created loader object. Null if no request has been done yet.
	 */
	public function getLoader() : ?TrLoader {
		return ($this->_loader);
	}
	/**
	 ...
	 * @return	\Trantor\Base\Loader	The dependency injection component.
	 */
	public function execLoader(string $url, string $httpMethod='GET', ?array $data=null, ?array $cookies=null) : \Trantor\Base\Loader {
		$trantor = $this->_createEnvironment($url, $httpMethod, $data);
		ob_start();
		$trantor->process(processView: null, sendHeaders: false);
		ob_end_clean();
		return ($this->_loader);
	}
	/**
	 ...
	 */
	public function execResponse(string $url, string $httpMethod='GET', ?array $data=null, ?array $cookies=null) : \Trantor\Web\Response {
		$trantor = $this->_createEnvironment($url, $httpMethod, $data);
		ob_start();
		$trantor->process(processView: null, sendHeaders: false);
		ob_end_clean();
	...
	public function execData(string $url, string $httpMethod='GET', ?array $data=null, ?array $cookies=null) : null|string|array {
		$trantor = $this->_createEnvironment($url, $httpMethod, $data);
		ob_start();
		$result = $trantor->process(processView: false, sendHeaders: false);
		ob_end_clean();
		return ($result);
	}
	/**
	...
	public function execOutput(string $url, string $httpMethod='GET', ?array $data=null, ?array $cookies=null) : string {
		$trantor = $this->_createEnvironment($url, $httpMethod, $data);
		ob_start();
		$trantor->process(sendHeaders: false);
		$result = ob_get_clean();
		return ($result ?: '');
	}
	 ...
	 * @param	?array	$data		(optional) Associative array of parameters (GET or POST parameters, depending on the $httpMethod parameter). Defaults to null.
	 * @param	?array	$cookies	(optional) Associative array of cookies. Defaults to null.
	 * @return	\Trantor\Web\Framework	An instance of the framework main object.
	 */
	protected function _createEnvironment(string $url, string $httpMethod='GET', ?array $data=null, ?array $cookies=null) : \Trantor\Web\Framework {
		// reset
		$_GET = $_POST = [];
		$_COOKIE = $cookies ?? [];
		...
		if ($this->_originalLoader)
			...
			$this->_loader = new TrLoader();
		// configuration loading
		if ($this->_appPath) {
			$config = new \Trantor\Web\Config($this->_appPath);
		...
		// request
		$this->_loader->request = new \Trantor\Web\Request($url);
		// framework
		$trantor = new \Trantor\Web\Framework($this->_loader);
		return ($trantor);
	}
}
