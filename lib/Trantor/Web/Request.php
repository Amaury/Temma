...
class Request {
	 ...
	 * @throws	\Trantor\Exceptions\Framework	If no PATH_INFO nor REQUEST_URI data found in the environment.
	 */
	public function __construct(null|bool|string $setUri=null) {
		TrLog::log('Trantor/Web', 'DEBUG', "Request creation.");
		if ($setUri === false)
			return;
		if (isset($setUri)) {
			...
			if (isset($_SERVER['REQUEST_URI']) && !empty($_SERVER['REQUEST_URI'])) {
				...
				throw new TrFrameworkException('No PATH_INFO nor REQUEST_URI environment variable.', TrFrameworkException::CONFIG);
			...
			// hint: PATH_INFO is not filled when we access to the Trantor project's root (being at the site root or in a sub-directory)
		...
		TrLog::log('Trantor/Web', 'INFO', "URL : '$requestUri'.");
