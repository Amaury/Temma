...
/**
 * Object used to store the configuration of a Trantor application.
 */
class Config {
	/** Prefix of the local configuration file. */
	const CONFIG_FILE_PREFIX = 'trantor';
	...
	const SESSION_NAME = 'TrantorSession';
	...
	const DEFAULT_LOADER = '\Trantor\Base\Loader';
	/** Name of the default controller. */
	const DEFAULT_CONTROLLER = '\Trantor\Web\Controller';
	/** Name of the default view. */
	const DEFAULT_VIEW = '\Trantor\Views\Smarty';
	/** Prefix of extended configuration section in the trantor.json file. */
	const XTRA_CONFIG_PREFIX = 'x-';
	/** Instance of the real configuration object that is used underneath this one. */
	protected ?\Trantor\Web\Config $_executorConfig = null;
	/** List of error pages. */
	 ...
	 * @return	\Trantor\Web\Config	The instanciated object.
	 */
	static public function __set_state(array $attributes) : \Trantor\Web\Config {
		$conf = new self($attributes['_appPath'], $attributes);
		return ($conf);
	}
	/**
	 * Reads the "trantor.json" configuration file.
	 * @param	?string	$forcedConfigPath	(optional) Path to the 'trantor.*' file to be used to get the configuration. Defaults to null, to get the default file 'etc/trantor.*'.
	 * @param	?array	$overConf		(optional) Associative array that contains data used to override the configuration file.
	 * @throws	\Trantor\Exceptions\Framework	If the configuration file is not correct.
	 */
	public function readConfigurationFile(?string $forcedConfigPath=null, ?array $overConf=null) : void {
		// read the configuration file
		if ($forcedConfigPath) {
			try {
				$ini = \Trantor\Utils\Serializer::read($forcedConfigPath);
			} catch (\Exception $e) {
				throw new TrFrameworkException("Unable to read configuration file '$forcedConfigPath'.", TrFrameworkException::CONFIG);
			}
		} else {
			// read the base configuration file ('etc/trantor.php', 'etc/trantor.json', 'etc/trantor.yaml' or 'etc/trantor.neon')
			$configPath = $this->_etcPath . '/' . self::CONFIG_FILE_PREFIX;
			$ini = null;
			try {
				$ini = \Trantor\Utils\Serializer::readFromPrefix($configPath);
			} catch (\Exception $e) { }
			// read the platform-specific configuration file (e.g. 'etc/trantor.prod.php', 'etc/trantor.staging.json', 'etc/trantor.dev.yaml', etc.)
			$envType = getenv(self::ENV_PLATFORM);
			if ($envType) {
				$envConfigPath = "$configPath.$envType";
				try {
					$envIni = \Trantor\Utils\Serializer::readFromPrefix($envConfigPath);
					$ini = \Trantor\Utils\ExtendedArray::fusion(($ini ?? []), $envIni);
				} catch (\Exception $e) { }
			}
			// verify if the configuration was found
			if (!$ini)
				throw new TrFrameworkException("Unable to read configuration file with prefix '$configPath'.", TrFrameworkException::CONFIG);
		}
		// overload if needed
		if ($overConf)
	...
	protected function _initIncludePaths() : void {
		...
		if ($pathsToInclude)
			\Trantor\Base\Autoload::addIncludePath($pathsToInclude);
	}
}
