...
class Controller implements \ArrayAccess {
	...
	protected \Trantor\Base\Loader $_loader;
	/** Session object. */
	protected ?\Trantor\Base\Session $_session = null;
	/** Config object. */
	protected ?\Trantor\Web\Config $_config = null;
	/** Request object. */
	protected ?\Trantor\Web\Request $_request = null;
	/** Response object. */
	protected ?\Trantor\Web\Response $_response = null;
	/** DAO object. */
	protected ?\Trantor\Dao\Dao $_dao = null;
	/** Configuration of auto DAO. */
	protected $_trantorAutoDao = null;

	/**
	 * Constructor.
	 * @param	\Trantor\Base\Loader	$loader		Dependency injection object, with (at least) the keys
	 *							'dataSources', 'session', 'config', 'request', 'response'.
	 * @param	\Trantor\Web\Controller	$executor	(optional) Executor controller object (the one who called this controller).
	 */
	public function __construct(\Trantor\Base\Loader $loader, ?\Trantor\Web\Controller $executor=null) {
		$this->_loader = $loader;
		...
		if (isset($this->_trantorAutoDao) && $this->_trantorAutoDao !== false) {
			$this->_dao = $this->_loadDao($this->_trantorAutoDao);
		}
	}
	/** Destructor. */
	 ...
	 * @return	\Trantor\Dao\Dao	The loaded DAO.
	 */
	public function _loadDao(null|bool|string|array $param) : \Trantor\Dao\Dao {
		$daoConf = [
			'object'   => '\Trantor\Dao\Dao',
			'criteria' => null,
			'source'   => null,
			'cache'    => true,
			'base'     => null,
		...
		if (isset($daoConf['source']) && isset($this->_dataSources[$daoConf['source']]))
			...
			$dataSource = $this->_loader->dataSources[\Trantor\Web\Framework::DEFAULT_DATASOURCE] ?? reset($this->_loader->dataSources);
		if (!is_a($dataSource, '\Trantor\Datasources\Sql'))
			throw new \Trantor\Exceptions\Database("DAO creation: The used data source is not of type '\\Trantor\Datasources\\Sql'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	 ...
	 * @return	\Trantor\Web\Controller	The current object.
	 */
	final protected function _view(string $view) : \Trantor\Web\Controller {
	 ...
	 * @return	\Trantor\Web\Controller	The current object.
	 */
	final protected function _template(string $template) : \Trantor\Web\Controller {
	 ...
	 * @return	\Trantor\Web\Controller	The current object.
	 */
	final protected function _templatePrefix(string $prefix) : \Trantor\Web\Controller {
	 ...
	 * @return	\Trantor\Web\Controller	The current object.
	 */
	protected function _header(string $header) : \Trantor\Web\Controller {
	 ...
	 * @throws	\Trantor\Exceptions\Http	If the requested controller or action doesn't exist.
	 * @throws	\Trantor\Exceptions\Flow	If the requested controller or action throws a Flow exception.
	 */
	final public function _subProcess(string $controller, ?string $action=null, ?array $parameters=null) : ?int {
		TrLog::log('Trantor/Web', 'DEBUG', "Subprocess of '$controller'::'$action'.");
		// checks
		if (!class_exists($controller) || !is_subclass_of($controller, '\Trantor\Web\Controller')) {
			TrLog::log('Trantor/Web', 'ERROR', "Sub-controller '$controller' doesn't exists.");
			throw new TrHttpException("Unable to find controller '$controller'.", 404);
		}
		...
		$method = \Trantor\Web\Framework::CONTROLLERS_INIT_METHOD;
		try {
			$status = $obj->$method();
		} catch (\Error $e) {
			TrLog::log('Trantor/Web', 'ERROR', "Unable to initialize the controller '$controller' [" . $e->getFile() . ':' . $e->getLine() . ']: ' . $e->getMessage());
			throw new TrHttpException("Unable to initialize the controller '$controller'.", 500);
		}
		...
		if (method_exists($controller, \Trantor\Web\Framework::CONTROLLERS_PROXY_ACTION)) {
			// proxy action found
			$method = \Trantor\Web\Framework::CONTROLLERS_PROXY_ACTION;
			$isProxyAction = true;
		} else if (method_exists($controller, \Trantor\Web\Framework::CONTROLLERS_OLD_PROXY_ACTION)) {
			// old proxy action found
			$method = \Trantor\Web\Framework::CONTROLLERS_OLD_PROXY_ACTION;
			$isProxyAction = true;
		} else {
			// no proxy action defined on this controller
			if (empty($action)) {
				// no action was requested, use the root action
				$method = \Trantor\Web\Framework::CONTROLLERS_ROOT_ACTION;
			} else if (!($firstLetter = $action[0]) || $firstLetter !== strtolower($firstLetter)) {
				// the action must start with a lower-case letter
				TrLog::log('Trantor/Web', 'ERROR', "Actions must start with a lower-case letter (here: '$action').");
				throw new TrHttpException("Actions must start with a lower-case letter (here: '$action').", 404);
			}
			// check that the action could be executed
			if (!is_callable([$obj, $action])) {
				// no proxy action, and no callable action method
				throw new TrHttpException("Unable to find action '$action' on controller '$controller'.", 404);
			}
		...
		$reflectionMethod = $isDefaultAction ? \Trantor\Web\Framework::CONTROLLERS_DEFAULT_ACTION : $method;
		...
		foreach ($attributes as $attribute) {
			TrLog::log('Trantor/Web', 'DEBUG', "Action attribute '{$attribute->getName()}'.");
		...
		try {
			...
			TrLog::log('Trantor/Web', 'ERROR', "$controller::$method: " . $ace->getMessage());
			throw new TrHttpException("$controller::$method: " . $ace->getMessage(), 404);
		} catch (\Error $e) {
			TrLog::log('Trantor/Web', 'ERROR', "$controller::$method" . '[' . $e->getFile() . ':' . $e->getLine() . ']: ' . $e->getMessage());
			throw new TrHttpException("Unable to execute method '$method' on controller '$controller'.", 404);
		}
		if ($status) // $status !== self::EXEC_FORWARD && $status !== self::EXEC_FORWARD_THROWABLE
			return ($status);

		/* ********** finalization ********** */
		$method = \Trantor\Web\Framework::CONTROLLERS_FINALIZE_METHOD;
		try {
			$status = $obj->$method();
		} catch (\Error $e) {
			TrLog::log('Trantor/Web', 'ERROR', "Unable to finalize the controller '$controller' [" . $e->getFile() . ':' . $e->getLine() . ']: ' . $e->getMessage());
			throw new TrHttpException("Unable to finalize the controller '$controller'.", 500);
		}
		return ($status);
	}
}
