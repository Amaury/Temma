 ...
 * @link	https://www.trantor.org/en/documentation/helper-ctrl_auth
 */

namespace Trantor\Controllers;

use \Trantor\Base\Log as TrLog;
use \Trantor\Attributes\Auth as TrAuth;
use \Trantor\Attributes\Methods\Post as TrPost;
use \Trantor\Utils\Email as TrEmail;
 ...
 * "x-security" extra-configuration (in the "trantor.json" file), under the "auth" key:
 * ```json
 * {
 *     "x-security": {
 *         "auth": {
 ...
 * ```
 */
class Auth extends \Trantor\Web\Plugin {
	/** User DAO. */
	private ?\Trantor\Dao\Dao $_userDao = null;
	/** Token DAO. */
	private ?\Trantor\Dao\Dao $_tokenDao = null;
	 ...
	 */
	#[TrAuth(authenticated: false, redirect: '/')]
	...
	public function authentication() {
		...
		if (!$email || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
			TrLog::log('Trantor/App', 'DEBUG', "Invalid email address '$email'.");
		...
		if (!isset($user['id'])) {
			// user not registered
			TrLog::log('Trantor/App', 'DEBUG', "Unknown email address '$email'.");
			...
			// register the user
			TrLog::log('Trantor/App', 'DEBUG', "Register user '$email'.");
			...
			if (!isset($user['id'])) {
				...
				TrLog::log('Trantor/App', 'DEBUG', "Unable to register user '$email'.");
				$this->_session['__authStatus'] = 'tokenSent';
				return (self::EXEC_HALT);
			}
		}
		...
		$token = \Trantor\Utils\BaseConvert::convertToSpecialBase($token, 16, 31);
		$token = substr($token, 0, 12);
		// store the token in database
		TrLog::log('arkiv', 'DEBUG', "Token : '$token'");
...
Best regards";
		...
		$this->_loader['\Trantor\Utils\Email']->textMail($sender, $email, $subject, $message);
	}
