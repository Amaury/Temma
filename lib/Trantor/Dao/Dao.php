...
namespace Trantor\Dao;

use \Trantor\Exceptions\Dao as TrDaoException;
...
class Dao {
	...
	protected string $_criteriaObject = '\Trantor\Dao\Criteria';
	/** Database connection. */
	protected \Trantor\Datasources\Sql $_db;
	/** Cache connection. */
	protected ?\Trantor\Base\Datasource $_cache;
	...
	/**
	 * Constructor.
	 * @param	\Trantor\Datasources\Sql	$db		Connection to the database.
	 * @param	?\Trantor\Base\Datasource	$cache		(optional) Connection to the cache server.
	 ...
	 * @param	?string			$criteriaObject	(optional) Name of the criteria object. (default: \Trantor\Dao\Criteria)
	 * @throws	\Trantor\Exceptions\Dao	If the criteria object is not of the right type.
	 */
	public function __construct(\Trantor\Datasources\Sql $db, ?\Trantor\Base\Datasource $cache=null, ?string $tableName=null,
		...
		if (!empty($criteriaObject)) {
			if (!is_subclass_of($criteriaObject, '\Trantor\Dao\Criteria'))
				throw new TrDaoException("Bad object type.", TrDaoException::CRITERIA);
	 ...
	 * @return	\Trantor\Datasources\Sql	The database object.
	 */
	public function getDataBase() : \Trantor\Datasources\Sql {
		return ($this->_db);
	}
	/**
	 * Returns the cache object.
	 * @return	?\Trantor\Base\Datasource	The cache object.
	 */
	public function getCache() : ?\Trantor\Base\Datasource {
		return ($this->_cache);
	}
	/**
	 ...
	 * This method should be used only by \Trantor\Dao\Criteria objects.
	 ...
	 * @return 	\Trantor\Dao\Criteria	The criteria object.
	 */
	public function criteria(string $type='and') : \Trantor\Dao\Criteria {
	 ...
	 * @param	null|array|\Trantor\Dao\Criteria	$criteria	(optional) Search criteria or an associative array of fields and their search values.
	 *								Null to count all records in the table. (default: null)
	 * @return	int	The number of records.
	 */
	public function count(null|array|\Trantor\Dao\Criteria $criteria=null) : int {
	 ...
	 * @param	int|string|array|\Trantor\Dao\Criteria	$id	Primary key or criteria.
	 * @return	array	Associative array.
	 */
	public function get(int|string|array|\Trantor\Dao\Criteria $id) : array {
	 ...
	 * @throws	\Trantor\Exceptions\Dao	If the input data are not well formed.
	...
	public function create(array $data, mixed $safeData=null) : int {
		...
		foreach ($data as $key => $value) {
			...
			if (is_null($value))
				...
				throw new TrDaoException("Bad field value for key '$key'.", TrDaoException::FIELD);
		}
		$dataSet = implode(', ', $set);
		$sql .= $dataSet;
	 ...
	 * @param	null|array|\Trantor\Dao\Criteria	$criteria	(optional) Search criteria or an associative array of fields and their search values. Null to take all records. (default: null)
	 ...
	 */
	public function search(null|array|\Trantor\Dao\Criteria $criteria=null, null|false|string|array $sort=null, ?int $limitOffset=null, ?int $nbrLimit=null) : array {
		$cacheVarName = '__dao:' . $this->_dbName . ':' . $this->_tableName . ':count';
		$sql = 'SELECT ' . $this->_getFieldsString() . ' FROM ' .
			(!$this->_dbName ? '' : ('`' . $this->_dbName . '`.')) . '`' . $this->_tableName . '`';
		if (isset($criteria)) {
	 ...
	 * @param	null|int|string|array|\Trantor\Dao\Criteria	$criteria	Primary key of the record that must be updated, or a search criteria.
	 ...
	 * @throws	\Trantor\Exceptions\Dao	If the criteria or the fields array are not well formed.
	 */
	public function update(null|int|string|array|\Trantor\Dao\Criteria $criteria=null, array $fields=[],
		...
		foreach ($fields as $field => $value) {
			...
			if (is_string($value) || is_int($value) || is_float($value))
				...
				throw new TrDaoException("Bad field '$field' value.", TrDaoException::VALUE);
		}
		$sql .= implode(',', $set);
		if (!is_null($criteria)) {
			$sql .= ' WHERE ';
	 ...
	 * @param	null|int|string|array|\Trantor\Dao\Criteria	$criteria	(optional) Primary key of the record that must be deleted, or a search criteria.
	 *										Null to remove all entries. (default: null)
	 * @return	int	The number of deleted lines.
	 */
	public function remove(null|int|string|array|\Trantor\Dao\Criteria $criteria=null) : int {
