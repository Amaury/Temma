 ...
 * @link	https://www.trantor.org/en/documentation/helper-registry
 */
 ...
 * $registry = new \Trantor\Utils\Registry();
...
class Registry implements \Iterator, \ArrayAccess {
	...
	public function __clone() {
		$this->_data = \Trantor\Utils\Misc::clone($this->_data);
	}
	/**
	 ...
	 * @return	\Trantor\Utils\Registry	The current object.
	 */
	public function reset(array $data=[]) : \Trantor\Utils\Registry {
		$this->_data = $data;
		return ($this);
	}
	 ...
	 * @return	\Trantor\Utils\Registry	The current object.
	 */
	public function set(string|array $keyOrArray, mixed $data=null) : \Trantor\Utils\Registry {
	 ...
	 * @return	\Trantor\Utils\Registry	The current object.
	 */
	public function unset(string $key) : \Trantor\Utils\Registry {
		$this->_data[$key] = null;
	 ...
	 */
	public function readIni(string $path, ?string $key=null) : \Trantor\Utils\Registry {
		// readINI file
		$result = parse_ini_file($path, true);
		if ($result === false) {
			TrLog::log('Trantor/Utils', 'WARN', "INI reading error.");
	 ...
	 */
	public function readJson(string $path, ?string $key=null) : \Trantor\Utils\Registry {
		// read JSON file
		try {
			$result = json_decode(file_get_contents($path), true, 512, JSON_THROW_ON_ERROR);
		} catch (\JsonException $e) {
			TrLog::log('Trantor/Utils', 'WARN', "JSON reading error: " . $e->getMessage());
			throw $e;
		}
		// store the JSON data
		if (is_null($key)) {
	 ...
	 * @return	\Trantor\Utils\Registry	The current object.
	 * @throws	\Trantor\Exceptions\IO	If the XML file can't be read.
	 */
	public function readXml(string $path, string $key) : \Trantor\Utils\Registry {
		// read XML file
		$xml = simplexml_load_file($path);
		if ($xml === false) {
			TrLog::log('Trantor/Utils', 'WARN', "Unable to read XML file '$path'.");
			throw new TrIOException("Unable to read XML file '$path'.", TrIOException::BAD_FORMAT);
		}
		// store the XML data
		$this->set($key, $xml);
		return ($this);
