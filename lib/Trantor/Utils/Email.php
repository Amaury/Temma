 ...
 * @link	https://www.trantor.org/en/documentation/helper-email
 */
 ...
 * TrEmail::simpleMail('luke@rebellion.org', 'vader@empire.com', "I can't beleive it", "You are my father?");
 * TrEmail::fullMail('vader@empire.com', 'luke@rebellion.org', "Yes", "<h1>Yes</h1><p>I am your father</p>");
 * ```
 *
 * The "trantor.json" configuration file may contain an "x-email" extended configuration,
 * to define automatic recipients ("cc" and "bcc"), and to define the envelope sender passed to sendmail.
 * ```json
 * {
 *     "x-email": {
 *         "disabled": true,
 *         "allowedDomains": ["trantor.org", "trantor.fr"],
 ...
 * $this->_loader['\Trantor\Utils\Email']->textMail( ... );
 * ```
 *
 * @link	https://www.php.net/manual/en/function.mail.php
 */
class Email implements \Trantor\Base\Loadable {
	...
	/**
	 * Constructor.
	 * @param	\Trantor\Base\Loader	$loader	Dependency injection component.
	 */
	...
	public function templatedMail(string $from, string|array $to, string $title='',
		...
		$html = $text = '';
		if ($htmlTplPath)
			$html = $this->_loader['\Trantor\Utils\Smarty']->render($htmlTplPath, $templateData);
		if ($textTplPath)
			$text = $this->_loader['\Trantor\Utils\Smarty']->render($textTplPath, $templateData);
