 ...
 * @link	https://www.trantor.net/en/documentation/helper-dumper
 */
...
class Dumper {
	 ...
	 * @return	\Trantor\Utils\Dumper	Instance of the object, configured for text dump.
	 */
	static public function factoryText() : \Trantor\Utils\Dumper {
		$dumper = new self();
		$dumper->setCallbacks(
	 ...
	 * @return	\Trantor\Utils\Dumper	Instance of the object, configured for ANSI dump.
	 */
	static public function factoryAnsi() : \Trantor\Utils\Dumper {
		$dumper = new self();
		$dumper->setCallbacks(
	 ...
	 * @return	\Trantor\Utils\Dumper	Instance of the object, configured for HTML dump.
	 */
	static public function factoryHtml() : \Trantor\Utils\Dumper {
		$dumper = new self();
		$dumper->setCallbacks(
	...
	public function _dumpAnsiRecursion() : string {
		return (TrAnsi::italic("RECURSION\n"));
	}
	/** ANSI null. */
	public function _dumpAnsiNull() : string {
		return (TrAnsi::bold(TrAnsi::color('yellow', "null\n")));
	}
	/** ANSI bool. */
	public function _dumpAnsiBool(bool $data) : string {
		$value = $data ? 'true' : 'false';
		return (TrAnsi::bold(TrAnsi::color('light-red', "$value\n")));
	}
	/** ANSI scalar. */
	public function _dumpAnsiScalar(mixed $data) : string {
		...
		if (str_starts_with($data, '"') && str_ends_with($data, '"')) {
			return (TrAnsi::color('yellow', '"') .
			        TrAnsi::color($color, mb_substr($data, 1, -1)) .
			        TrAnsi::color('yellow', '"') . "\n");
		}
		return (TrAnsi::color($color, "$data\n"));
	}
	/** ANSI array start. */
	public function _dumpAnsiArrayStart(array $data) : string {
		$this->_indentation++;
		$this->_arrayCounterStack[] = 0;
		return (TrAnsi::color('green', 'array') . ' ' .
		        TrAnsi::color('yellow', ($data ? '[' : '[]')) . "\n");
	}
		...
		if ($key == end($this->_arrayCounterStack)) {
			...
			if (is_string($key)) {
				...
				if (str_starts_with($key, '"') && str_ends_with($key, '"')) {
					$res .= TrAnsi::color('yellow', '"') .
						TrAnsi::color('magenta', mb_substr($key, 1, -1)) .
						TrAnsi::color('yellow', '"');
				} else
					$res .= TrAnsi::color('magenta', $key);
			} else {
				$this->_arrayCounterStack[count($this->_arrayCounterStack) - 1] = $key + 1;
				$res .= TrAnsi::color('light-blue', (string)$key);
			}
			$res .= TrAnsi::color('yellow', ' => ');
		}
		return ($res);
	...
	public function _dumpAnsiObjectStart(object $data, string $objectName, array $properties) : string {
		$this->_indentation++;
		return (
			TrAnsi::color('green', $objectName) .
			TrAnsi::color('gray', ' #' . spl_object_id($data)) .
			TrAnsi::color('yellow', ($properties ? ' {' : ' {}')) .
			"\n"
		);
	}
	...
	public function _dumpAnsiPropertyStart(mixed $value, string $propertyName, string $visibility, string $staticness) : string {
		...
		$res = str_repeat('    ', $this->_indentation) .
		       TrAnsi::color('yellow', $visibility);
		if ($staticness == 'static')
			$res .= TrAnsi::underline($propertyName);
		else
			$res .= $propertyName;
		$res .= TrAnsi::color('yellow', ': ');
		return ($res);
	...
	protected function _addToKnownList(mixed $data) : bool {
		if (is_array($data))
			$id = md5(TrSerializer::encode($data, TrSerializer::JSON));
