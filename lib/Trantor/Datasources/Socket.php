 ...
 * @link	https://www.trantor.org/en/documentation/datasource-socket
 */
 ...
 * <code>
 * // initialization
 * $sock = \Trantor\Datasources\Socket::factory('socket://HOST:PORT');
 * $sock = \Trantor\Base\Datasource::factory('socket://HOST:PORT');
 *
 ...
 * </code>
 */
class Socket extends \Trantor\Base\Datasource {
	 ...
	 * @return	\Trantor\Datasources\Socket	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Socket {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\Socket object creation with DSN: '$dsn'.");
		$type = null;
		$host = null;
		$port = null;
		$timeout = null;
	 ...
	 * @throws	\Trantor\Exceptions\Database	If the socket type is invalid.
	 */
	public function __construct(string $type, string $host, ?int $port, ?int $timeout) {
		if (!in_array($type, self::SOCKET_TYPES)) {
			throw new \Trantor\Exceptions\Database("Invalid socket type '$type'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		$this->_type = $type;
		$this->_host = $host;
		$this->_port = $port ?? -1;
	...
	public function mRemove(array $keys) : void {
		throw new \Trantor\Exceptions\Database("No mRemove() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	 * Disable clear().
