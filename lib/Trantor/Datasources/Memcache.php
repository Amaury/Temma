 ...
 * @link	https://www.trantor.org/en/documentation/datasource-memcache
 */
 ...
 * <code>
 * // init
 * $cache = \Trantor\Datasources\Memcache::factory('memcache://localhost');
 * // alternative init
 * $cache = \Trantor\Base\DataSource::factory('memcache://localhost');
 *
 ...
 */
class Memcache extends \Trantor\Base\Datasource implements \ArrayAccess {
	 ...
	 * @return	\Trantor\Datasources\Memcache	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is not valid or if the 'memcached' extension is not loaded.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Memcache {
		if (!str_starts_with($dsn, 'memcache://') ||
		    !($servers = mb_substr($dsn, mb_strlen('memcache://')))) {
			throw new \Trantor\Exceptions\Database("The DSN '$dsn' is not valid.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
	 ...
	 * @throws	\Trantor\Exceptions\Database	If the given server configuration is wrong or if the 'memcached' extension is not loaded.
	 */
	public function __construct(array $servers) {
		if (!extension_loaded('memcached')) {
			throw new \Trantor\Exceptions\Database("The 'memcached' PHP extension is not loaded.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
	...
	public function reconnect() {
		...
		if ((count($this->_servers) == 1 && !$this->_memcache->addServer($this->_servers[0][0], $this->_servers[0][1])) ||
			...
			$this->_memcache = null;
			$this->_enabled = false;
			throw new \Trantor\Exceptions\Database("Unable to connect to Memcache server.", \Trantor\Exceptions\Database::CONNECTION);
		}
	}
	/** Disconnection. */
	 ...
	 * @return	\Trantor\Datasources\Memcache	The current object.
	 */
	public function setExpiration(int $expiration=86400) : \Trantor\Datasources\Memcache {
	 ...
	 * @return	\Trantor\Datasources\Memcache	The current object.
	 */
	public function setPrefix(string $prefix='') : \Trantor\Datasources\Memcache {
	 ...
	 * @throws	\Trantor\Exceptions\Database	If the data is not a string.
	 */
	public function read(string $key, mixed $defaultOrCallback=null, mixed $expire=0) : ?string {
		...
		if (is_string($data) || $data === null)
			return ($data);
		throw new \Trantor\Exceptions\Database("Data '$key' is not of type string.", \Trantor\Exceptions\Database::TYPE);
	}
	/*
	 * Multiple read.
	 * @param	array	$keys	List of keys.
