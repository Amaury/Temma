 ...
 * @link	https://www.trantor.org/en/documentation/datasource-beanstalk
 */
 ...
 * <code>
 * // initialization
 * $bean = \Trantor\Datasources\Beanstalk::factory('beanstalk://HOST:PORT/TUBE_NAME');
 * $bean = \Trantor\Base\Datasource::factory('beanstalk://HOST:PORT/TUBE_NAME');
 *
 ...
 * </code>
 */
class Beanstalk extends \Trantor\Base\Datasource {
	/** Pheanstalk object. */
	 ...
	 * @return	\Trantor\Datasources\Beanstalk	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Beanstalk {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\Beanstalk object creation with DSN: '$dsn'.");
		if (!preg_match('/^beanstalk:\/\/([^\/:]+):?([^\/]+)?\/(.*)$/', $dsn, $matches)) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid Beanstalk DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid Beanstalk DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		$host = $matches[1] ?? null;
		$port = $matches[2] ?? null;
		$tube = $matches[3] ?? '';
	 ...
	 * @return	\Trantor\Datasources\Beanstalk	The current object.
	 */
	public function touch(string $id) : \Trantor\Datasources\Beanstalk {
		if (!$this->_enabled)
			return ($this);
		$this->connect();
	...
	public function mRead(array $keys) : array {
		throw new \Trantor\Exceptions\Database("No mRead() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
