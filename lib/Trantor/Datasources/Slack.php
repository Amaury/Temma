 ...
 * @link	https://www.trantor.org/en/documentation/datasource-slack
 */
 ...
 * <code>
 * // initialization
 * $slack = \Trantor\Datasources\Slack::factory('slack://WEBHOOK_URL');
 * $slack = \Trantor\Base\Datasource::factory('slack://WEBHOOK_URL');
 ...
 * $slack[''] = [
 *     'text'        => 'Text above the notification <https://trantor.org|with a link>',
 *     'attachments' => [
 *         [
 ...
 * ];
 */
class Slack extends \Trantor\Base\Datasource {
	/** Webhook URL. */
	 ...
	 * @return	\Trantor\Datasources\Slack	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Slack {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\Slack object creation with DSN: '$dsn'.");
		$webhook = null;
		if (!str_starts_with($dsn, 'slack://')) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid Slack DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid Slack DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		...
		if (!str_starts_with($dsn, 'hooks.slack.com/services/'))
			throw new \Trantor\Exceptions\Database("Invalid Slack webhook '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		$webhook = "https://$dsn";
	...
	public function set(string $channel, mixed $message=null, mixed $options=null) : bool {
		...
		if (is_string($message))
			...
			throw new \Trantor\Exceptions\Database("Bad Slack message parameter.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		...
		curl_close($curl);
		if ($result != 'ok')
			throw new \Trantor\Exceptions\Database("Bad Slack response '$result'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		return (true);
	}
	/**
	 * Disable mSet().
