 ...
 * @link	https://www.trantor.org/en/documentation/datasource-s3
 */

namespace Trantor\Datasources;
 ...
 * $s3 = \Trantor\Datasources\S3::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET');
 * $s3 = \Trantor\Base\Datasource::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET');
 * // alternative init
 * $s3 = \Trantor\Datasources\S3::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET:private');
 * $s3 = \Trantor\Base\Datasource::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET:private');
 *
 * // initialization, with public-read access for created files by default
 * $s3 = \Trantor\Datasources\S3::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET:public');
 * $s3 = \Trantor\Base\Datasource::factory('s3://ACCESS_KEY:SECRET_KEY@REGION/BUCKET:public');
 *
 ...
 * </code>
 */
class S3 extends \Trantor\Base\Datasource {
	 ...
	 * @return	\Trantor\Datasources\S3	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\S3 {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\S3 object creation with DSN: '$dsn'.");
		if (!preg_match("/^([^:]+):\/\/([^:]+):([^@]+)@([^\/]+)\/([^:]+):?(.*)$/", $dsn, $matches)) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid S3 DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid S3 DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		$type = $matches[1] ?? null;
		$accessKey = $matches[2] ?? '';
	 ...
	 * @throws	\Trantor\Exceptions\Database	If a parameter is invalid.
	 */
	public function __construct(string $accessKey, string $privateKey, string $region, string $bucket, bool $publicAccess=false) {
		...
		if (!$this->_accessKey || !$this->_privateKey || !$this->_region || !$this->_bucket)
			throw new \Trantor\Exceptions\Database("Bad S3 connection parameters.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	...
	/**
	 * Reconnection.
	 * @throws	\Trantor\Exceptions\Database	If the connection failed.
	 */
	...
	public function count() : int {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to list files on AWS S3. Bucket='{$this->_bucket}'.");
		}
		return ($count);
	}
	...
	public function getUrl(string $s3Path) : string {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to create S3 pre-signed URL. Bucket='{$this->_bucket}'. key='$s3Path'.");
			$presignedUrl = null;
		}
		return ($presignedUrl);
	}
	...
	public function remove(string $s3Path) : void {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to remove AWS S3 file. Bucket='{$this->_bucket}'. key='$s3Path'.");
			throw $e;
		}
	}
	/**
	...
	public function mRemove(array $s3Paths) : void {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to remove AWS S3 files. Bucket='{$this->_bucket}'.");
			throw $e;
		}
	}
	/**
	...
	public function clear(string $prefix) : void {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to remove AWS S3 files. Bucket='{$this->_bucket}'. prefix='$prefix'.");
			throw $e;
		}
	}
	/**
	 ...
	 * @throws	\Trantor\Exceptions\IO	If the destination path is not writable.
	 * @throws	\Exception		If an error occured.
	 */
	...
	public function write(string $s3Path, string $data, mixed $options=null) : bool {
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Can't send data to AWS S3. Bucket='{$this->_bucket}'. key='$s3Path'.");
			throw $e;
		}
		return (true);
	}
	 ...
	 *					If an array: 'public' (bool) and/or 'mimetype' (string) keys.
	 * @return	bool	Always true.
	 * @throws	\Trantor\Exceptions\IO		If the destination file is not writeable.
	 * @throws	\Exception	If an error occured.
	 */
	public function copyTo(string $s3Path, string $localPath, mixed $options=null) : bool {
		...
		if (!is_writeable($localPath)) {
			TrLog::log('Trantor/Base', 'INFO', "Unable to write in file '$localPath'.");
			throw new \Trantor\Exceptions\IO("Unable to write in file '$localPath'.", \Trantor\Exceptions\IO::UNWRITABLE);
		}
		// check parameters
		$public = $this->_publicAccess;
		$mimetype = 'application/octet-stream';
		...
		try {
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to send file to AWS S3. Bucket='{$this->_bucket}'. key='$s3Path'.");
			throw $e;
		}
		return (true);
	}
