 ...
 * @link	https://www.trantor.org/en/documentation/datasource-sql
 */

namespace Trantor\Datasources;

use \Trantor\Base\Log as TrLog;
use \Trantor\Exceptions\Database as TrDatabaseException;
 ...
 *     $db = new \Trantor\Base\Datasource("mysql://user:pwd@localhost/database");
 *     // simple request
 ...
 * <code>
 * // object creation, database connection
 * try { $db = new \Trantor\Base\Datasource::factory("mysql://user:pwd@localhost/database"); }
 * catch (Exception $e) { }
 ...
 * </code>
 *
 * For use as a regular datasource, you must create a table named 'TrantorData':
 * <code>
 * CREATE TABLE TrantorData (
 ...
 * </code>
 */
class Sql extends \Trantor\Base\Datasource {
	 ...
	 * @return	\Trantor\Datasources\Sql	The created object.
	 * @throws	\Trantor\Exceptions\Database	If something went wrong.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Sql {
		// parameters extraction
		$params = null;
		if (!preg_match("/^([^:]+):\/\/([^:@]+):?([^@]+)?@([^\/:]+):?(\d+)?\/([^#]*)#?(.*)$/", $dsn, $matches)) {
			throw new \Trantor\Exceptions\Database("Invalid SQL DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		$type = $matches[1];
		$login = $matches[2];
		$password = $matches[3];
	...
	public function __construct(string $type, ?string $login=null, ?string $password=null, ?string $host=null, ?string $port=null, ?string $base=null, ?string $socket=null) {
		TrLog::log('Trantor/Base', 'DEBUG', "SQL database object creation.");
		// check type
		if (!in_array($type, self::DATABASE_TYPES)) {
			throw new \Trantor\Exceptions\Database("Invalid SQL type '$type'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		// set parameters
	...
	public function reconnect() {
		...
		try {
			...
			throw new \Trantor\Exceptions\Database("SQL Database connection error: " . $e->getMessage(), \Trantor\Exceptions\Database::CONNECTION);
		}
	}
	/** Disconnection. */
	...
	public function transaction(callable $callback) : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Starting a transaction.");
	...
	public function startTransaction() : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Beginning transaction.");
		if (!$this->_enabled)
			return;
		$this->connect();
		if ($this->_db->beginTransaction() === false) {
			TrLog::log('Trantor/Base', 'ERROR', "Unable to start a new transaction.");
	...
	public function commit() : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Committing transaction.");
		if (!$this->_enabled)
			return;
		$this->connect();
		if ($this->_db->commit() === false) {
			TrLog::log('Trantor/Base', 'ERROR', "Error during transaction commit.");
	...
	public function rollback() : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Rollbacking transaction.");
		if (!$this->_enabled)
			return;
		$this->connect();
		if ($this->_db->rollback() === false) {
			TrLog::log('Trantor/Base', 'ERROR', "Error during transaction rollback.");
	 ...
	 * @return	\Trantor\Datasources\SqlStatement	The statement object.
	 * @throws	\Trantor\Exceptions\Database	If an error occurs.
	 */
	public function prepare(string $sql) : \Trantor\Datasources\SqlStatement {
		TrLog::log('Trantor/Base', 'DEBUG', "SQL prepare: $sql");
		if (!$this->_enabled)
			throw new TrDatabaseException("Unable to prepare a query while the connection is disabled.", TrDatabaseException::QUERY);
		$this->connect();
		try {
			$dbStatement = $this->_db->prepare($sql);
		} catch (\PDOException $pe) {
			TrLog::log('Trantor/Base', 'ERROR', 'Database prepare error: ' . $pe->getMessage());
			throw new TrDatabaseException($pe->getMessage(), TrDatabaseException::QUERY);
		}
		if ($dbStatement === false) {
			$errStr = 'Database prepare error: ' . $this->getError();
			TrLog::log('Trantor/Base', 'ERROR', $errStr);
			throw new TrDatabaseException($errStr, TrDatabaseException::QUERY);
		}
		return (new \Trantor\Datasources\SqlStatement($this, $dbStatement));
	}
	/**
	...
	public function exec(string $sql, bool $buffered=false) : int {
		TrLog::log('Trantor/Base', 'DEBUG', "SQL query: $sql");
		if (!$this->_enabled)
			return (0);
		/*
	...
	public function flush() : void {
		if (!$this->_enabled)
			return;
		$sql = "TRUNCATE TABLE TrantorData";
		$this->exec($sql);
	}
	...
	public function mWrite(array $data, mixed $options=null) : int {
		...
		$sql = "INSERT INTO TrantorData (key, data)
