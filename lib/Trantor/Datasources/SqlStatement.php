...
class SqlStatement {
	...
	/* ********** CONSTRUCTION ********** */
	/**
	 * Constructor. Should be used only by \Trantor\Base\Database object.
	 * @param	\Trantor\Datasources\Sql	$db		The datbase object.
	 * @param	\PDOStatement 			$statement	The PDOStatement object.
	 */
	public function __construct(\Trantor\Datasources\Sql $db, \PDOStatement $statement) {
		$this->_db = $db;
	...
	public function exec(?array $parameters=null) : int {
		if (!$this->_statement->execute($parameters)) {
			...
			throw new TrDatabaseException($err, TrDatabaseException::QUERY);
		}
