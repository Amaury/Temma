 ...
 * @link	https://www.trantor.org/en/documentation/datasource-discord
 */
 ...
 * <code>
 * // initialization
 * $discord = \Trantor\Datasources\Discord::factory('discord://WEBHOOK_URL');
 * $discord = \Trantor\Base\Datasource::factory('discord://WEBHOOK_URL');
 ...
 */
class Discord extends \Trantor\Base\Datasource {
	/** Webhook URL. */
	 ...
	 * @return	\Trantor\Datasources\Discord	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Discord {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\Discord object creation with DSN: '$dsn'.");
		$webhook = null;
		if (!str_starts_with($dsn, 'discord://')) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid Discord DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid Discord DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		...
		if (!str_starts_with($dsn, 'discord.com/api/webhooks/'))
			throw new \Trantor\Exceptions\Database("Invalid Discord webhook '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		$webhook = "https://$dsn";
	 ...
	 * @param	mixed		$options	Not used.
	 * @return	bool	always true.
	 * @throws	\Trantor\Exceptions\Database	If an error occured.
	 */
	public function set(string $channel, mixed $message=null, mixed $options=null) : bool {
		...
		if (is_string($message))
			...
			throw new \Trantor\Exceptions\Database("Bad Discord message parameter.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		// send the message
		...
		curl_close($curl);
		if ($result != '')
			throw new \Trantor\Exceptions\Database("Bad Discord response '$result'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		return (true);
	}
	/**
	 * Disable mSet().
	...
	public function mSet(array $data, mixed $options=null) : int {
		throw new \Trantor\Exceptions\Database("No mSet() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
}
