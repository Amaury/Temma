 ...
 * @link	https://www.trantor.org/en/documentation/datasource-smsmode
 */
 ...
 * <code>
 * // initialization
 * $sms = \Trantor\Datasources\Smsmode::factory('smsmode://API_KEY');
 * $sms = \Trantor\Base\Datasource::factory('smsmode://API_KEY');
 *
 * // initialization with a sender name (11 characters-long maximum, no space, no accent)
 * $sms = \Trantor\Datasources\Smsmode::factory('smsmode://SENDER_NAME:API_KEY');
 * $sms = \Trantor\Base\Datasource::factory('smsmode://SENDER_NAME:API_KEY');
 *
 ...
 * if ($status == \Trantor\Datasources\Smsmode::STATUS_SUCCESS) {
 *     // ...
 * } else if ($status == \Trantor\Datasources\Smsmode::STATUS_RECEIVED) {
 *     // ...
 * } else if ($status == \Trantor\Datasources\Smsmode::STATUS_RECEIVED) {
 *     // ...
 * } else if ($status == \Trantor\Datasources\Smsmode::STATUS_GATEWAY_RECEIVED ||
 *            $status == \Trantor\Datasources\Smsmode::STATUS_GATEWAY_DELIVERED) {
 *     // ...
 * } else if ($status == \Trantor\Datasources\Smsmode::STATUS_INTERNAL_ERROR ||
 *            $status == \Trantor\Datasources\Smsmode::STATUS_DELIVERY_ERROR) {
 *     // ...
 * }
 *
 ...
 * </code>
 */
class Smsmode extends \Trantor\Base\Datasource {
	 ...
	 * @return	\Trantor\Datasources\Smsmode	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\Smsmode {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\Smsmode object creation with DSN: '$dsn'.");
		if (!preg_match('/^smsmode:\/\/(([^:]*):)?(.+)$/', $dsn, $matches)) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid Smsmode DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid Smsmode DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
	...
	public function send(string|array $recipient, string $text=null, ?string $sendDate=null, ?string $reference=null) : ?string {
		...
		// unicode
		$unicode = false;
		if (!\Trantor\Utils\Text::encodingCompatible($text, 'iso-8859-15'))
			$unicode = true;
		// parameters
		$params = [
	...
	public function clear(string $pattern) : void {
		throw new \Trantor\Exceptions\Database("No clear() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	...
	private function _request(string $endpoint, array $parameters=[], bool $unicode=false) : string {
		...
		TrLog::log('Trantor/Base', 'DEBUG', "Smsmode request: '$url'.");
