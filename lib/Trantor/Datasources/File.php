 ...
 * @link	https://www.trantor.org/en/documentation/datasource-file
 */
 ...
 * $file = \Trantor\Datasources\File::factory('file:///opt/trantor_files');
 * $file = \Trantor\Base\Datasource::factory('file:///opt/trantor_files');
 * // init with a custom umask
 * $file = \Trantor\Datasources\File::factory('file://0007/opt/trantor_files');
 * $file = \Trantor\Base\Datasource::factory('file://0007/opt/trantor_files');
 *
 ...
 * </code>
 */
class File extends \Trantor\Base\Datasource {
	/** Root path. */
	 ...
	 * @return	\Trantor\Datasources\File	The created instance.
	 * @throws	\Trantor\Exceptions\Database	If the DSN is invalid.
	 */
	static public function factory(string $dsn) : \Trantor\Datasources\File {
		TrLog::log('Trantor/Base', 'DEBUG', "\\Trantor\\Datasources\\File object creation with DSN: '$dsn'.");
		if (!preg_match("/^([^:]+):\/\/([^\/]+)\/(.*)$/", $dsn, $matches)) {
			TrLog::log('Trantor/Base', 'WARN', "Invalid File DSN '$dsn'.");
			throw new \Trantor\Exceptions\Database("Invalid File DSN '$dsn'.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		$type = $matches[1] ?? null;
		$umask = $matches[2] ?? '';
		$rootPath = $matches[3] ?? '';
	...
	public function write(string $path, string $data, mixed $options=0) : bool {
		...
		if (!is_writeable($fullPath)) {
			TrLog::log('Trantor/Base', 'INFO', "Unable to write file '$fullPath'.");
			throw new \Trantor\Exceptions\IO("Unable to write file '$fullPath'.", \Trantor\Exceptions\IO::UNWRITABLE);
		}
		// manage permissions
		$permissions = null;
		if ($options) {
	...
	public function copyTo(string $path, string $localPath, mixed $options=0) : bool {
		...
		if (!copy($localPath, $fullPath)) {
			TrLog::log('Trantor/Base', 'INFO', "Unable to copy file '$localPath' to '$fullPath'.");
			throw new \Trantor\Exceptions\IO("Unable to copy file '$localPath' to '$fullPath'.", \Trantor\Exceptions\IO::UNWRITABLE);
		}
		return (true);
	}
