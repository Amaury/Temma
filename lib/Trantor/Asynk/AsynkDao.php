 ...
 */
class AsynkDao implements \Trantor\Base\Loadable {
	...
	protected ?\Trantor\Base\Datasource $_transport = null;
	/** Storage data source. */
	protected ?\Trantor\Base\Datasource $_storage = null;
	 ...
	 * @throws	\Trantor\Exceptions\Framework	If the configuration is not valid.
	 */
	public function __construct(\Trantor\Base\Loader $loader) {
		...
		if ((!$transport && in_array($storageClass, self::STORAGE_NO_TRANSPORT)) ||
			...
			throw new TrFrameworkException("Invalid storage '$storage' with '$transport' transport.", TrFrameworkException::CONFIG);
		}
		if ($storageClass == 'Trantor\Datasources\Sql') {
			$daoClass = $loader->config->xtra('asynk', 'dao', '\Trantor\Dao\Dao');
	 ...
	 * @throws	\Trantor\Exceptions\Framework	If the configuration is incorrect.
	 */
	public function createTask(string $target, string $action, array $params) : void {
		$taskId = null;
		// storage
		if ($this->_storageClass == 'Trantor\Datasources\Sql') {
			// MySQL storage (with or without transport)
			$taskId = $this->_storeTaskSql($target, $action, $params);
		} else if ($this->_storageClass == 'Trantor\Datasources\Redis') {
			// Redis storage
			if ($this->_transport) {
			...
			throw new TrFrameworkException("Unknown Asynk storage '{$this->_storageClass}'.", TrFrameworkException::CONFIG);
		}
		// transport
		if (!$this->_transport) {
			// crontab
		...
		if (in_array($this->_transportClass, self::TRANSPORT_NOTIFY)) {
			...
			throw new TrFrameworkException("Unknown Asynk transport '{$this->_transportClass}'.", TrFrameworkException::CONFIG);
		}
	}
	/**
	...
	public function getTaskFromId(int|string $taskId) : ?array {
		...
		if (!$this->_storage)
			return (null);
		if ($this->_storageClass != 'Trantor\Datasources\Redis')
			return (null);
		/* Redis storage */
		// fetch the task
	...
	public function getNextTask() : ?array {
		...
		// xinetd transport
		if ($this->_transportClass == 'Trantor\Datasources\Socket')
			return (null);
		// Redis queue
	...
	private function _storeTaskKeyValue(string $target, string $action, array $params) : string {
		...
		$s = \Trantor\Utils\BaseConvert::convert($s, 16, 62);
