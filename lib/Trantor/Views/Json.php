 ...
 * @link	http://json.org/
 */
class Json extends \Trantor\Web\View {
	...
	public function init() : void {
		...
		// output filtering
		if ($contract)
			$this->_data = \Trantor\Utils\DataFilter::process($this->_data, $contract);
	}
	/** Write HTTP headers. */
	public function sendHeaders(?array $headers=null) : void {
		if ($this->_filename) {
			$headers ??= [];
			$headers[] = 'Content-Disposition: attachment; filename="' . \Trantor\Utils\Text::filenamize($this->_filename) . '"';
		}
