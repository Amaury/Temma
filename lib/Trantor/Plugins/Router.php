...
/**
 * Plugin used extends the Trantor routing feature.
 *
 * @see		https://www.trantor.org/fr/documentation/routage
 */
class Router extends \Trantor\Web\Plugin {
	 ...
	 * @throws	\Trantor\Exceptions\Framework	If the configuration is not correct.
	 */
	public function preplugin() {
		TrLog::log('Trantor/Web', 'INFO', "Router plugin started.");
		...
		if (!(isset($this->_routes[$currentMethod]) && $this->_searchRoute($this->_routes[$currentMethod], $chunks)) &&
			...
			TrLog::log('Trantor/Web', 'DEBUG', "Router: no route found.");
		...
		if (is_array($this->_exec)) {
			...
			if (!is_string($this->_exec['action'] ?? null)) {
				TrLog::log('Trantor/Web', 'WARN', "Router: Bad configuration (missing 'action' key).");
				throw new TrFrameworkException("Router: Bad configuration (missing 'action' key).", TrFrameworkException::CONFIG);
			}
		...
		if (count($res) != 2) {
			TrLog::log('Trantor/Web', 'WARN', "Router: Bad configuration (missing '::' separator between object and method).");
			throw new TrFrameworkException("Router: Bad configuration (missing '::' separator between object and method).", TrFrameworkException::CONFIG);
		}
		...
		if (($pos = mb_strpos($method, '(')) === false) {
			...
			if (!str_ends_with($method, ')')) {
				TrLog::log('Trantor/Web', 'WARN', "Router: Bad configuration (missing ')' at the end of method expression).");
				throw new TrFrameworkException("Router: Bad configuration (missing ')' at the end of method expression)).", TrFrameworkException::CONFIG);
			}
			...
			while (($chunk = array_shift($urlChunks))) {
				if ($chunk[0] == '[') {
					...
					if (count($data) < 2 || !in_array($data[1], ['int', 'float', 'string', 'enum'])) {
						TrLog::log('Trantor/Web', 'WARN', "Router: Bad typed parameter '$chunk'.");
						throw new TrFrameworkException("Router: Bad typed parameter '$chunk'.", TrFrameworkException::CONFIG);
					}
					[$name, $type] = $data;
					$params = $data[2] ?? null;
					$values = null;
