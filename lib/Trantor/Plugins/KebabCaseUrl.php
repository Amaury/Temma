 ...
 *
 * @see	\Trantor\Utils\Text::convertCase
 */
class KebabCaseUrl extends \Trantor\Web\Plugin {
	/** Preplugin method. */
	public function preplugin() {
		...
		if (TrText::hasUpper($oldController) || TrText::hasUpper($oldAction)) {
			TrLog::log('Trantor/Web', 'ERROR', "Controller or action in camel case. Kebab case required.");
		...
		if (!str_contains($oldController, '-') && !str_contains($oldAction, '-'))
			return;
		// controller rewrite
		$controller = TrText::convertCase($oldController ?? '', TrText::KEBAB_CASE, TrText::CAMEL_CASE);
		$this->_loader->request->setController($controller);
		// action rewrite
		$action = TrText::convertCase($oldAction ?? '', TrText::KEBAB_CASE, TrText::CAMEL_CASE);
