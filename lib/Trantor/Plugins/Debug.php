...
use \Trantor\Utils\Text as TrText;
use \Trantor\Utils\Dumper as TrDumper;

/**
 * Plugin used to set debug toolbar.
 */
class Debug extends \Trantor\Web\Plugin {
	...
	/** Dumper object. */
	static protected TrDumper $_dumper;
	 ...
	 */
		...
		// store logs
		if (self::$_showLogs) {
			TrLog::addCallback(function($id, $message, $priority, $class) {
				if ($priority) {
	...
	protected function _generateHtml(float $time) : string {
		...
		if (self::$_showLogs) {
			...
			if (!self::$_logs) {
				...
				if (!$debugFound)
					$html .= "<br>Set a <a href='https://www.trantor.org/en/documentation/configuration#doc-loglevels' target='_blank'>log level</a> to <tt>DEBUG</tt> to get log messages..";
				$html .= "</p>";
			} else {
				$html .= "<table>\n";
				if (self::$_logLevelCounts || self::$_logClasses) {
					...
					if (self::$_logLevelCounts) {
						...
						foreach (TrLog::LEVELS as $level => $value) {
							if (!isset(self::$_logLevelCounts[$level]))
								continue;
							$html .= "<option value='" . TrText::urlize($level) . "'>" . htmlspecialchars($level) . " (" . self::$_logLevelCounts[$level] . ")</option>";
						}
						$html .= "	</select>
							</td>";
					}
					if (self::$_logClasses) {
						...
						foreach (self::$_logClasses as $class => $value) {
							$html .= "<option value='" . TrText::urlize($class) . "'>" . htmlspecialchars($class) . " ($value)</option>";
						}
						$html .= "	</select>
							</td>";
					}
				...
				foreach (self::$_logs as $log) {
					$cssClass = ($log['level'] ? '_tµ-log-' . TrText::urlize($log['level']) : '') . ' ' .
						    ($log['class'] ? '_tµ-log-class-' . TrText::urlize($log['class']) : '');
					$color = in_array($log['level'], ['CRIT', 'ERROR']) ? 'red' :
						 ($log['level'] == 'WARN' ? 'orange' :
						  (in_array($log['level'], ['NOTE', 'INFO']) ? '#383' : '#666'));
					$html .= "<tr valign='top' class='_tµ-log _tµ-log-" . TrText::urlize($log['level']) . " _tµ-log-class-" . TrText::urlize($log['class']) . "'>
					...
					$classBg = '#555';
					$classFg = '#fff';
					if ($log['class'] == 'Trantor/Base')
						$classBg = $colorMedium;
					else if ($log['class'] == 'Trantor/Web')
						$classBg = $colorDark;
					else if (str_starts_with($log['class'], 'Trantor/')) {
						$classBg = $colorLight;
						$classFg = $colorDark;
					}
					if (self::$_logClasses)
		...
		if (self::$_showVariables) {
			...
			if (!$tplVars) {
				...
				$html .= "<table>";
				ksort($tplVars);
				self::$_dumper = TrDumper::factoryHtml();
		...
		if (self::$_showBar) {
			...
			BAR;
			$controllerName = $this->_loader->trantor->getControllerName();
			$actionName = $this->_loader->trantor->getActionName();
			...
			$version = \Trantor\Web\Framework::TRANTOR_VERSION;
			$html .= <<<BAR
						</span>
						<span style='color: #999;'>|</span>
						<span title='Trantor version'>
							<a href="https://github.com/Digicreon/Trantor/releases/tag/$version" target="_blank" style="color: #ddd;">v$version</a>
						</span>
					</span>
				</div>
			BAR;
