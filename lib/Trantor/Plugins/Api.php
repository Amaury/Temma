 ...
 * Then, controllers can use the Auth attribute (see https://www.trantor.org/documentation/attribut-auth)
 ...
 * ```
 *
 * @link	https://www.trantor.org/documentation/plugin-api
 */
class Api extends \Trantor\Web\Plugin {
	/** User DAO. */
	private ?\Trantor\Dao\Dao $_userDao = null;
	/** ApiKey DAO. */
	private ?\Trantor\Dao\Dao $_apiKeyDao = null;

        /**
	 * Preplugin method.
	 * @return	mixed	EXEC_HALT if a redirection is needed. EXEC_FORWARD otherwise.
	 * @throws	\Trantor\Exceptions\FlowHalt	If the URL is not well formed.
	 */
        public function preplugin() {
		// check if the current user has already been fetch by another plugin
		if ($this['currentUser']) {
			TrLog::log('Trantor/Web', 'WARN', "The user has already been defined.");
			return;
		}
		// special headers
		header('Access-Control-Allow-Origin: *');
		// JSON view
		$this->_view('\Trantor\Views\Json');
	...
	static public function generateKeys() : array {
		...
		while (mb_strlen($public, 'ascii') < 32) {
			...
			$key = hash('sha256', $key);
			$key = \Trantor\Utils\BaseConvert::convertToSpecialBase($key, 16, 71);
			$public .= $key;
		}
	 ...
	 * @throws	\Trantor\Exceptions\FlowHalt	If the URL is ont well formed.
	 */
	private function _manageUrl() : void {
		...
		if ($version[0] != 'v' || !ctype_digit(mb_substr($version, 1))) {
			TrLog::log('Trantor/Web', 'WARN', "Incorrect API version number '$version'.");
			$this->_httpError(400);
