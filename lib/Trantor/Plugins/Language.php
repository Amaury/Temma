 ...
 *
 * @see	https://www.trantor.fr/fr/documentation/plugin-language
 */
class Language extends \Trantor\Web\Plugin {
        /**
	 * Plugin method.
	 ...
	 */
		...
		if (!$defaultLanguage || !is_array($supportedLanguages) || !is_array($protectedUrls)) {
			...
			TrLog::log('Trantor/Web', 'WARN', "Wrong configuration for language plugin.");
			return (self::EXEC_FORWARD);
		}
		...
		if (!in_array($currentLang, $supportedLanguages)) {
			...
			if (in_array(($_SERVER['REQUEST_METHOD'] ?? null), ['POST', 'PUT'])) {
				TrLog::log('Trantor/Web', 'DEBUG', "No language required for POST/PUT request.");
				return (self::EXEC_FORWARD);
			}
			// no language required for protected URLs
			if (in_array($this['URL'], $protectedUrls)) {
				TrLog::log('Trantor/Web', 'DEBUG', "No language required for protected URL.");
				return (self::EXEC_FORWARD);
			}
		...
		try {
			$langFile = TrSerializer::readFromPrefix($langPath);
		} catch (TrIOException $ioe) {
			$logLevel = ($currentLang == $defaultLanguage) ? 'NOTE' : 'WARN';
			TrLog::log('Trantor/Web', $logLevel, "Unable to read language file '$langPath'.");
			$langFile = null;
		}
