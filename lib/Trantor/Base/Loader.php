 ...
 * @link	https://www.trantor.org/en/documentation/loader
 */
 ...
 * <code>
 * // create a loader
 * $loader = new \Trantor\Base\Loader();
 ...
 * $loader = new \Trantor\Base\Loader(['userBo' => $userBo], function($loader, $key) {
 ...
 * <code>
 * class MyLoader extends \Trantor\Base\Loader {
 ...
 * </code>
 */
class Loader extends \Trantor\Utils\Registry {
	/** Builder function. */
	 ...
	 * @return	\Trantor\Base\Loader	The current object.
	 */
	public function setBuilder(callable $builder) : \Trantor\Base\Loader {
		$this->_builder = $builder;
		return ($this);
	}
	 ...
	 * @return	\Trantor\Base\Loader	The current object.
	 */
	public function set(string|array $key, mixed $data=null) : \Trantor\Base\Loader {
		if (is_array($key))
	...
	public function get(string $key, mixed $default=null) : mixed {
		...
		if ($key == 'asynk') {
			return (new \Trantor\Asynk\Client($this));
		}
		// key exists
		if (isset($this->_data[$key])) {
			if (is_callable($this->_data[$key]) && !$this->_data[$key] instanceof \Trantor\Web\Controller)
		...
		if (isset($this->_aliases[$key])) {
			...
			// store in data
			if ((is_callable($alias) && !$alias instanceof \Trantor\Web\Controller) || is_string($alias)) {
			...
			foreach ($this->_prefixes as $prefix => $value) {
				...
				if (is_callable($value) && !$value instanceof \Trantor\Web\Controller) {
					// it's a callback, execute it
	...
	private function _instanciate(string|callable $obj, mixed $default=null) : mixed {
		$result = null;
		// callable
		if (is_callable($obj) && !$obj instanceof \Trantor\Web\Controller) {
			$result = $obj($this);
			if (isset($result))
				return ($result);
		}
		// loadable
		if (($interfaces = @class_implements($obj)) && isset($interfaces['Trantor\Base\Loadable'])) {
			return (new $obj($this));
		}
		// DAO
		if (is_subclass_of($obj, '\Trantor\Dao\Dao') && isset($this->_data['controller']) && $this->_data['controller'] instanceof \Trantor\Web\Controller) {
			return ($this->_data['controller']->_loadDao($obj));
		}
		// controller
		if (is_subclass_of($obj, '\Trantor\Web\Controller')) {
			...
			foreach ($attributes as $attribute) {
				TrLog::log('Trantor/Web', 'DEBUG', "Controller attribute '{$attribute->getName()}'.");
		...
		if (is_callable($default) && !$default instanceof \Trantor\Web\Controller) {
