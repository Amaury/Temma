 ...
 * $session = \Trantor\Base\Session::factory($cache);
 *
...
class Session implements \ArrayAccess {
	...
	private ?\Trantor\Base\Datasource $_cache = null;
	...
	/**
	 * Factory.
	 * @param	\Trantor\Base\Datasource	$cache		(optional) Cache management object.
	 * @param	string				$cookieName	(optional) Name of the session cookie. "TRANTOR_SESSION" by default.
	 ...
	 *								- other = use the defined value as the cookie domain
	 * @return	\Trantor\Base\Session	The created instance.
	 */
	static public function factory(?\Trantor\Base\Datasource $cache=null, string $cookieName='TRANTOR_SESSION',
	                               int $duration=31536000, null|false|string $domain=null) : \Trantor\Base\Session {
		return (new \Trantor\Base\Session($cache, $cookieName, $duration, $domain));
	}
	/**
	 * Constructor.
	 * @param	\Trantor\Base\Datasource	$cache		(optional) Cache management object.
	 ...
	 *								- other = use the defined value as the cookie domain
	 */
	private function __construct(?\Trantor\Base\Datasource $cache=null, ?string $cookieName='TRANTOR_SESSION',
	                             ?int $duration=null, null|false|string $domain=null) {
		TrLog::log('Trantor/Base', 'DEBUG', "Session object creation.");
		$this->_domain = $domain;
		// fetch the cache
	...
	public function sendCookie() : void {
		...
		if (!isset($_COOKIE[$this->_cookieName]) || empty($_COOKIE[$this->_cookieName]) || $this->_sessionId != $oldSessionId && !headers_sent()) {
			TrLog::log('Trantor/Base', 'DEBUG', "Send cookie '{$this->_cookieName}' - '{$this->_sessionId}' - '$timestamp' - '.$host'");
			$options = [
				'expires'	=> $timestamp,
				'path'		=> '/',
	...
	public function clean() : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Cleaning session.");
	...
	public function remove() : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Removing current session.");
		if (!isset($this->_cache)) {
			// standard PHP session
	...
	public function set(string $key, mixed $value=null) : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Setting value for key '$key'.");
		if (!isset($this->_cache)) {
			if (is_null($value))
				unset($_SESSION[$key]);
			else
	...
	public function setArray(string $key, string $arrayKey, mixed $value=null) : void {
		TrLog::log('Trantor/Base', 'DEBUG', "Setting value for array '$key\[$arrayKey\]'.");
		if (!isset($this->_cache)) {
			if (is_null($value)) {
	...
	public function extract(string $key, mixed $default=null) : mixed {
		TrLog::log('Trantor/Base', 'DEBUG', "Extracting value for key '$key'.");
	...
	public function get(string $key, mixed $default=null) : mixed {
		TrLog::log('Trantor/Base', 'DEBUG', "Returning value for key '$key'.");
		if (!isset($this->_cache)) {
