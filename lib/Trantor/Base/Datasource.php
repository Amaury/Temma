...
abstract class Datasource implements \ArrayAccess, \Countable {
	 ...
	 * @return	\Trantor\Base\Datasource	The created object.
	 * @throws	\Trantor\Exceptions\Database	If the given parameters are not correct.
	 */
	static public function metaFactory(string|array $param) : \Trantor\Base\Datasource {
		// check this method is not called on subclasses
		if (get_called_class() != __CLASS__) {
			TrLog::log('Trantor/Base', 'ERROR', "Method metaFactory() must not be called on subclasses.");
			throw new \Trantor\Exceptions\Database("Method metaFactory() must not be called on subclasses.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		// manage DSN string
		if (is_string($param))
		...
		if (!($object = $param['object'])) {
			TrLog::log('Trantor/Base', 'ERROR', "Unable to find 'object' key on data source parameter array.");
			throw new \Trantor\Exceptions\Database("Unable to find 'object' key on data source parameter array.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		if (!is_a($object, '\Trantor\Base\Datasource', true)) {
			TrLog::log('Trantor/Base', 'ERROR', "Object '$object' is not a valid data source object.");
			throw new \Trantor\Exceptions\Database("Object '$object' is not a valid data source object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
		}
		unset($param['object']);
	 ...
	 */
	static public function factory(string $dsn) : \Trantor\Base\Datasource {
		TrLog::log('Trantor/Base', 'DEBUG', "Datasource object creation with DSN: '$dsn'.");
		// manage specified data source objects
		if (preg_match('/\[([^\]]+)\](.*)$/', $dsn, $matches)) {
			$object = $matches[1];
			$dsn = $matches[2];
			if (is_a($object, '\Trantor\Base\Datasource', true)) {
				TrLog::log('Trantor/Web', 'DEBUG', "Load data source '$object'.");
				return ($object::factory($dsn));
			}
		}
		// SQL databases
		foreach (\Trantor\Datasources\Sql::DATABASE_TYPES as $prefix) {
			if (str_starts_with($dsn, "$prefix:"))
				return (\Trantor\Datasources\Sql::factory($dsn));
		}
		// socket connexion
		foreach (\Trantor\Datasources\Socket::SOCKET_TYPES as $prefix) {
			if (str_starts_with($dsn, "$prefix:"))
				return (\Trantor\Datasources\Socket::factory($dsn));
		}
		// other data sources managed by Trantor
		if (str_starts_with($dsn, 'memcache://'))
			return (\Trantor\Datasources\Memcache::factory($dsn));
		if (str_starts_with($dsn, 'redis://') ||
		    str_starts_with($dsn, 'redis-sock://'))
			return (\Trantor\Datasources\Redis::factory($dsn));
		if (str_starts_with($dsn, 'file://'))
			return (\Trantor\Datasources\File::factory($dsn));
		if (str_starts_with($dsn, 's3://'))
			return (\Trantor\Datasources\S3::factory($dsn));
		if (str_starts_with($dsn, 'zmq://') ||
		    str_starts_with($dsn, 'zmq-bind://'))
			return (\Trantor\Datasources\ZeroMQ::factory($dsn));
		if (str_starts_with($dsn, 'sqs://'))
			return (\Trantor\Datasources\Sqs::factory($dsn));
		if (str_starts_with($dsn, 'beanstalk://'))
			return (\Trantor\Datasources\Beanstalk::factory($dsn));
		if (str_starts_with($dsn, 'smsmode://'))
			return (\Trantor\Datasources\Smsmode::factory($dsn));
		if (str_starts_with($dsn, 'slack://'))
			return (\Trantor\Datasources\Slack::factory($dsn));
		if (str_starts_with($dsn, 'pushover://'))
			return (\Trantor\Datasources\Pushover::factory($dsn));
		if (str_starts_with($dsn, 'discord://'))
			return (\Trantor\Datasources\Discord::factory($dsn));
		if (str_starts_with($dsn, 'dummy://'))
			return (\Trantor\Datasources\Dummy::factory(''));
	 ...
	 * @return	\Trantor\Base\Datasource	The current object.
	 */
	public function enable() : \Trantor\Base\Datasource {
		$this->_enabled = true;
		return ($this);
	}
	/**
	 ...
	 * @return	\Trantor\Base\Datasource	The current object.
	 */
	public function disable() : \Trantor\Base\Datasource {
		$this->_enabled = false;
		return ($this);
	}
	 ...
	 * @throws	\Trantor\Exceptions\Database	Always throws an exception.
	 */
	public function isSet(string $key) : bool {
		throw new \Trantor\Exceptions\Database("No isSet() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	...
	public function remove(string $key) : void {
		throw new \Trantor\Exceptions\Database("No remove() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	 * Multiple remove.
	...
	public function flush() : void {
		throw new \Trantor\Exceptions\Database("No flush() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	...
	public function find(string $pattern, bool $getValues=false) : array {
		throw new \Trantor\Exceptions\Database("No find() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	...
	public function read(string $key, mixed $defaultOrCallback=null, mixed $options=null) : mixed {
		throw new \Trantor\Exceptions\Database("No read() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	 * Multiple read.
	 ...
	 * @throws	\Trantor\Exceptions\IO	If the local file is not writable.
	 */
	public function copyFrom(string $key, string $localPath, mixed $defaultOrCallback=null, mixed $options=null) : bool {
		...
		if (($dirname && !file_exists($dirname) && !mkdir($dirname, 0777, true)) ||
			...
			TrLog::log('Trantor/Base', 'INFO', "Unable to write file '$localPath'.");
			throw new \Trantor\Exceptions\IO("Unable to write file '$localPath'.", \Trantor\Exceptions\IO::UNWRITABLE);
		}
		// get the file's data
	...
	public function write(string $key, string $value, mixed $options=null) : mixed {
		throw new \Trantor\Exceptions\Database("No set() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/**
	 * Multiple write.
	 ...
	 * @param	mixed	$options	(optional) Options (like expiration duration).
	 * @return	mixed	Implementation-dependent.
	 * @throws	\Trantor\Exceptions\IO	If the local file is not readable.
	 */
	public function copyTo(string $key, string $localPath, mixed $options=null) : mixed {
		if (!$this->_enabled)
			return ($this);
		if (!is_readable($localPath))
			throw new \Trantor\Exceptions\IO("Unable to read file '$localPath'.", \Trantor\Exceptions\IO::UNREADABLE);
	...
	public function count() : int {
		throw new \Trantor\Exceptions\Database("No count() method on this object.", \Trantor\Exceptions\Database::FUNDAMENTAL);
	}
	/** 
