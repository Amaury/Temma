 ...
 * \Trantor\Base\Log::log("Log message.");
 * // set the message criticity
 * \Trantor\Base\Log::log('WARN', "Warning message.");
 * // call from global context (not from inside a function nor a method)
 * \Trantor\Base\Log::fullLog('ERROR', "Error message.", __FILE__, __LINE__);
 * </code>
 *
 * <b>Log thresholds</b>
 *
 ...
 * \Trantor\Base\Log::setThreshold('INFO');
 * // this message will not be written
 * \Trantor\Base\Log::log('DEBUG', "Debug message.");
 * // this message will be written
 * \Trantor\Base\Log::log('NOTE', "Notification");
 * </code>
 *
 * The path to the log file is set by calling the \Trantor\Base\setLogFile($path) method.
 *
 * <b>Log classes</b>
 *
 ...
 * \Trantor\Base\Log::setThreshold('ERROR');
 * // this message will not be written
 * \Trantor\Base\Log::log('WARN', "Classless message.");
 * // definition of classes and their associated thresholds
 * $thresholds = [
 *         'default' => 'ERROR',
 *         'testing' => 'DEBUG',
 * ];
 * \Trantor\Base\Log::setThreshold($thresholds);
 * // add another class, with its associated threshold
 * \Trantor\Base\Log::setThreshold('foo', 'INFO');
 * // this message will be written
 * \Trantor\Base\Log::log('default', 'ERROR', "Message using the default class.");
 * // this message will be written too
 * \Trantor\Base\Log::log('testing', 'CRIT', "Critical message.");
 * </code>
 */
class Log {
	 ...
	 * @throws	\Trantor\Exceptions\IO		If there was a writing error.
	 */
	static private function _writeLog(?string $class, ?string $priority, string $message) : void {
		...
		if (self::$_logPath) {
			...
			if (file_put_contents($path, $text, $flags) === false)
				throw new TrIOException("Unable to write on log file '$path'.", TrIOException::UNWRITABLE);
		}
	}
}
